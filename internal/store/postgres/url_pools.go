...
func (s *urlPoolStore) Get(ctx context.Context, id string) (*model.URLPool, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id,name,type,description,urls_json,created_at,updated_at FROM url_pools WHERE id=$1`, id)
...
func (s *urlPoolStore) Update(ctx context.Context, p *model.URLPool) error {
	...
	_, err := s.db.ExecContext(ctx, `
		UPDATE url_pools
		SET name=$1, type=$2, description=$3, urls_json=$4, updated_at=$5
		WHERE id=$6`,
...
func (s *urlPoolStore) Delete(ctx context.Context, id string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM url_pools WHERE id=$1`, id)
	return err
}
